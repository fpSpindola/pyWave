...
from pywave.decoder import path_to_songname
from pywave import PyWave
from pywave.fingerprint import *
import traceback
import fnmatch
import os
import re
        ...
        for f in self.test_files:
            ...
            if result.strip() == "None":
                ...
                if song_result != song:
                    ...
                    self.result_query_duration[line][col] = round(result[PyWave.MATCH_TIME], 3)
                    self.result_match_confidence[line][col] = result[PyWave.CONFIDENCE]

                    song_start_time = re.findall("_[^_]+", f)
                    song_start_time = song_start_time[0].lstrip("_ ")

                    result_start_time = round((result[PyWave.OFFSET] * DEFAULT_WINDOW_SIZE *
                    ...
                    log_msg('query duration: %s' % round(result[PyWave.MATCH_TIME], 3))
                    log_msg('confidence: %s' % result[PyWave.CONFIDENCE])
