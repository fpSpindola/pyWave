...
# scan files
log_msg("Running PyWave fingerprinter on files in %s..." % test_folder,
        log=options.log, silent=options.silent)

tm = time.time()
djv = DejavuTest(options.temp_folder, test_seconds)
log_msg("finished obtaining results from pywave in %s" % (time.time() - tm),
        log=options.log, silent=options.silent)
